...
import static vn.thanh.metadataservice.repository.specification.SearchOperation.*;
...
public class EntitySpecificationsBuilder<T> {
    ...
    public Specification<T> build() {
        ...
        for (int i = 1; i < params.size(); i++) {
            ...
            result = params.get(i).isOrPredicate() ? Specification.allOf(result).or(new EntitySpecification<>(params.get(i))) : Specification.allOf(result).and(new EntitySpecification<>(params.get(i)));
        }
        return result;
    }
