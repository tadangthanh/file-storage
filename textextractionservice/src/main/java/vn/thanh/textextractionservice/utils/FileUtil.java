package vn.thanh.textextractionservice.utils;
...
import vn.thanh.textextractionservice.exception.CustomIOException;
...
public class FileUtil {
    ...
    public static String extractTextByType(String mimeType, String fileName, InputStream is) {
        try {
            ...
            return switch (mimeType) {
                case "application/msword" -> extractDocWithFallback(data, ext);
                case "application/vnd.openxmlformats-officedocument.wordprocessingml.document" ->
                        extractDocxText(new ByteArrayInputStream(data));
                case "application/pdf" -> extractPdfText(new ByteArrayInputStream(data));
                case "text/plain" -> extractTxtText(new ByteArrayInputStream(data));
                case "application/vnd.ms-excel" -> extractXlsText(new ByteArrayInputStream(data));
                case "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" ->
                        extractExcelText(new ByteArrayInputStream(data));
                case "application/vnd.ms-powerpoint" -> extractPptText(new ByteArrayInputStream(data));
                case "application/vnd.openxmlformats-officedocument.presentationml.presentation" ->
                        extractPptxText(new ByteArrayInputStream(data));
                default ->
                    // Fallback to extension-based detection
                        extractByExtension(ext, new ByteArrayInputStream(data));
            };
        } catch (IOException e) {
    ...
    private static String extractByExtension(String ext, InputStream is) {
        switch (ext) {
            ...
            case "xls":
                return extractXlsText(is);
            case "xlsx":
                return extractExcelText(is);
            case "ppt":
            ...
            case "txt":
                return extractTxtText(is);
            default:
                throw new CustomIOException("Unsupported file extension: " + ext);
        }
    }
